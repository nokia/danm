...
var pushResultInterfaces = []*current.Interface {
      &current.Interface{Name: "eth0",},
      &current.Interface{Name: "eth1",},
      &current.Interface{Name: "fastpath",},
    }

var pushResultTestConsts = []result {
  {"sriov", nil, &current.Result{CNIVersion: "0.3.1", Interfaces: pushResultInterfaces },},
  {"flannel", errors.New("this did not go well"), nil},
  {"ipvlan", errors.New("neither did this"), &current.Result{CNIVersion: "0.3.1", Interfaces: pushResultInterfaces },},
}

var aggregatedResultTestConsts = []result {
  {"sriov", nil, &current.Result{CNIVersion: "0.3.1", Interfaces: pushResultInterfaces },},
  {"flannel", nil, nil},
  {"ipvlan", nil, &current.Result{CNIVersion: "0.4.0", Interfaces: pushResultInterfaces },},
}
...
func TestPushResult(t *testing.T) {
  syncher := setupTest(len(pushResultTestConsts), pushResultTestConsts)
  if syncher.ExpectedNumOfResults != len(pushResultTestConsts) {
    t.Errorf("Expected number of stored results in object:%d does not match with the initalized value:%d", syncher.ExpectedNumOfResults, len(pushResultTestConsts))
  }
  if len(syncher.CniResults) != len(pushResultTestConsts) {
    t.Errorf("Number of stored results in object:%d does not match with the number we have pushed:%d", len(syncher.CniResults), len(pushResultTestConsts))
  }
  for index, result := range pushResultTestConsts {
...
func TestGetAggregatedResultSuccess(t *testing.T) {
  syncher := setupTest(len(aggregatedResultTestConsts)+1, aggregatedResultTestConsts)
...
func TestMergeCniResults(t *testing.T) {
  ...
  for _, result := range pushResultTestConsts {
...
func TestWasAnyOperationErroneous(t *testing.T) {
  ...
  failSyncher := setupTest(len(pushResultTestConsts), pushResultTestConsts)
  ...
  successSyncher := setupTest(len(aggregatedResultTestConsts), aggregatedResultTestConsts)
